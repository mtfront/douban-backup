...
// Solve douban pow challenge https://github.com/bambooom/douban-backup/commit/ba7e14117f36f9084ac391ac4dbc40c8b452553d
// 创建一个持久化的 CookieJar 实例，建议放在函数外部以维持会话
const cookieJar = new CookieJar();

// 辅助函数：解 PoW 难题
async function solveDoubanPoW(html) {
  const dom = new JSDOM(html);
  const doc = dom.window.document;

  const tok = (doc.querySelector('#tok'))?.value;
  const cha = (doc.querySelector('#cha'))?.value;
  const red = (doc.querySelector('#red'))?.value;
